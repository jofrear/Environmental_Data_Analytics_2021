...
EPAair <- read.csv("./Data/Processed/EPAair_O3_PM25_NC1819_Processed_A04.csv", stringsAsFactors = TRUE)
# Set date to date format
...
Air_model <- lm(data = EPAair, formula = PM2.5 ~ Ozone)
summary(Air_model)
...
Air_mod_plot <- ggplot(data = EPAair, aes(x = Ozone, y = PM2.5)) + geom_point() + geom_smooth(method = "lm")
print(Air_mod_plot)
```
...
Ozone.mod.full <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + meanlongitude + meanlatitude)
summary(Ozone.mod.full)
step(Ozone.mod.full)
...
Ozone.mod.best <- lm(formula = PM2.5 ~ Ozone + Month + meanlongitude + meanlatitude, 
    data = EPAair)
summary(Ozone.mod.best)
#Ex3 R-squared = 0.1316, Ex7 R-squared = 0.1926
```