...
## double brackets returns value as numeric instead of as a list
list1[[2]]
...
```{r}
temp_durham_high_F <- c(53, 55, 54, 49, 53, 56, 53, 42, 37, 42)
temp_durham_low_F <- c(25, 41, 32, 32, 28, 36, 26, 16, 26, 20)
...
```{r}
temp_durham_high_C <- (temp_durham_high_F - 32) * 5/9
temp_durham_high_C 
temp_durham_low_C <- (temp_durham_low_F - 32) * 5/9
temp_durham_low_C
...
highs_C <- to_celsius(temp_durham_high_F)
lows_C <- to_celsius(temp_durham_low_F)
...
```{r}
dataframe_temps <- data.frame(temp_durham_low_F, temp_durham_high_F, temp_durham_low_C, temp_durham_high_C)
names(dataframe_temps) <- c("Low (F)", "High (F)", "Low (C)", "High (C)")
...
```{r}
summary(dataframe_temps)
sd(dataframe_temps$`High (F)`)
