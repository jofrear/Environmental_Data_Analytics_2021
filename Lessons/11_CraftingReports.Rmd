...
* Easy to organize
* Ability to format
* Can mix **code** and **text**
* Less clunky
* Reproducible
* Version Control (vs *Word* or *Excel*)
* Knit into various formats
...
| Dataset     | Description |
| ----------- | ----------- |
| Ozone       | Time series data of ozone concentrations in Garinger |
| NTL-LTER    | Data from Peter and Paul Lakes LTER sites |
| NIWO_Litter | Litterfall data from NEON NIWO site |
...
NTL_LTER_Lake_Nutrients_Raw$sampledate <- as.Date(NTL_LTER_Lake_Nutrients_Raw$sampledate, "%m/%d/%y")
```
...
NTL_LTER_Lake_Nutrients_Raw <- NTL_LTER_Lake_Nutrients_Raw %>% 
...
Lake_Nitrogen <- NTL_LTER_Lake_Nutrients_Raw %>% 
    group_by(lakename) %>% 
    summarize(mean(tn_ug), min(tn_ug), max(tn_ug), sd(tn_ug))

Lake_Phosphorus <- NTL_LTER_Lake_Nutrients_Raw %>% 
    group_by(lakename) %>% 
    summarize(mean(tp_ug), min(tp_ug), max(tp_ug), sd(tp_ug))
```

```{r echo=FALSE}

kable(Lake_Nitrogen, caption = "Total Nitrogen, by Lake")
kable(Lake_Phosphorus, caption = "Total Phosphorus, by Lake")
...
```{r echo=FALSE, fig.cap="Total Nitrogen (ppm)", fig.align='left'}

ggplot(NTL_LTER_Lake_Nutrients_Raw, aes(x = sampledate, y = tn_ug, color = lakename)) + geom_point() + labs(x = "Sample Date", y = "Nitrogen", color = "Lake")

```

```{r echo=FALSE, fig.cap="Total Phosphorus (ppm)", fig.align='left'}

ggplot(NTL_LTER_Lake_Nutrients_Raw, aes(x = sampledate, y = tp_ug, color = lakename)) + geom_point() + labs(x = "Sample Date", y = "Phosphorus", color = "Lake")
...
Phosphorus and Nitrogen have been increasing in the lakes sampled over the course of the study period, with a spike in 1996.

---------------------------------------------------------

Next steps of analysis could include running a time series analysis on each lake to see if there is a trend. 
