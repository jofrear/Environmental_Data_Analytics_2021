 ...
 * Spatial data structures: sfg, sfc, sf objects
      sf = spatial features object
      sfc = spatial feature column in sf
      sfg = one row of the sfc
...
summary(wind_energy)
#wind_energy$huc8 <- substring(wind_energy$HUC_12, first = 1, last = 8)

windE_grouped <- wind_energy %>% 
  mutate(HUC_8 = substring(HUC_12, first = 1, last = 8)) %>% 
  group_by(HUC_8) %>% 
  summarize(kWhkm2day_mean = mean(kWhkm2day))

#Join to HUC_8 features
huc8_sf_wind <- huc8_sf %>% left_join(windE_grouped, by = c('HUC_8'))

#View the outputs
mapview(huc8_sf_wind, zcol = "kWhkm2day_mean")
```
